...
swww img ${wall_dir}/${wall_selection} --transition-type=center --transition-duration=2
wal -i ${wall_dir}/${wall_selection}
cp $HOME/.cache/wal/colors-waybar.css $HOME/.config/wlogout/
swaync-client -rs
pkill -9 waybar && ~/.config/waybar/scripts/launch-waybar.sh
cp ${wall_dir}/${wall_selection} $HOME/.config/swww/current_wallpaper.${wall_selection##*.}
magick $HOME/.config/swww/current_wallpaper.${wall_selection##*.} $HOME/.config/swww/current_wallpaper.png
